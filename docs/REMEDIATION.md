...
- **Status:** Verified (2026-05-07). `mosquitto/config/mosquitto.conf` flips `allow_anonymous` to `false` and adds `password_file /mosquitto/config/passwd` + `acl_file /mosquitto/config/acl`. `install.sh` section 6 provisions the passwd file by computing PBKDF2-SHA512 (1000 iter, 64-byte salt, 64-byte derived key — Mosquitto v2 `$7$` format) directly in Node (`crypto.pbkdf2Sync`). The earlier `docker run … mosquitto_passwd` approach was abandoned because the host-side file ended up owned by the invoking user (UID 1000) with mode 640, and the broker container's `mosquitto` user (UID 1883) could not read it; the Node-based hashing produces a 644 file the broker can read regardless of UID. `docker-compose.yml` exposes `MQTT_USERNAME`/`MQTT_PASSWORD` to the broker; healthcheck authenticates with them. README Quick Start points at `./install.sh`. `mosquitto/config/passwd` in `.gitignore`. 3-gate integration: full `FM_UNATTENDED=1 ./install.sh` clean + anonymous mosquitto_sub refused + authenticated mosquitto_sub returns broker uptime. Coverage: `backend/tests/mosquitto-config.test.js` (5 cases).
...
- **Effort:** S.
- **Status:** Verified (2026-05-07). Pure validator at `backend/src/services/opcua-endpoint-validator.js`; `opcua-bridge.js::start()` invokes it before `OPCUAClient.connect()` and refuses to start the bridge on reject (logs ERROR, backend continues without OPC UA). `OPCUA_ALLOWED_HOSTS` added to Joi schema + `.env.example`. Coverage: `backend/tests/opcua-endpoint-validator.test.js` 17 cases. Live verification: invoked from inside the running `factorymind-backend` container — `opc.tcp://169.254.169.254:4840/` (AWS metadata) returns `{ok:false, reason:"Host metadata-service vietato"}`; `opc.tcp://plc01.factory.local:4840/` returns `{ok:true}`.
- **Why this remediation, not another:**
...
- **Effort:** S.
- **Status:** Verified (2026-05-07). `.github/workflows/ci.yml` security job: `|| true` removed from both `npm audit` invocations; Trivy step pinned to `severity: HIGH,CRITICAL` + `exit-code: 1`; Gitleaks defaults to non-zero exit on detection. Regression coverage in `backend/tests/ci-security-gates.test.js` (5 cases). Live verification: `npm audit --production --audit-level=high` against a `lodash@4.17.10` fixture exits RC=1 with "1 critical severity vulnerability" reported (multiple GHSAs incl. Command Injection); against the actual backend it exits RC=0 — gate is calibrated and would fail a real PR introducing a HIGH/CRITICAL CVE.
...
| R-TF-STATE-001 | W1 | Code complete | 2026-05-07 | — | terraform/versions.tf backend "s3" uncommented + terraform/bootstrap-state.sh shipped (S3 versioning + KMS + public-access-block + TLS-only policy + cross-account-deny + lifecycle; DDB pay-per-request + SSE + PITR + deletion-protection). `terraform validate` clean. Verified flips after operator runs bootstrap-state.sh + terraform init -migrate-state in the AWS account. |
